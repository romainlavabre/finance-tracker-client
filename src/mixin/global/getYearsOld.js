...
export default function (date, reference = new Date().toISOString()) {
    date = LocalDate.of(new Date(date).getFullYear(), new Date(date).getMonth() + 1, new Date(date).getDay());
    reference = LocalDate.of(new Date(reference).getFullYear(), new Date(reference).getMonth() + 1, new Date(reference).getDay() === 0 ? 1 : new Date(reference).getDay());

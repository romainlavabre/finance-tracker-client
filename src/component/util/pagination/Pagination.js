...
export default function Pagination({
    ...
    useEffect(() => {
        ...
        const paginationFetch = e => {
            if (!isNull(e.data.name) && e.data.name !== name) {
                return;
            }
        ...
        eventDispatcher.subscribe(event.PAGINATION_FETCH, paginationFetch);
        ...
        return () => {
            ...
            eventDispatcher.unsubscribe(event.PAGINATION_FETCH, paginationFetch);
        };
    }, []);
        ...
        axios.get(fetch.url + getComputedQueryString(filtersRef.current, columns, fetch, mode), fetch.options)
            .then(response => {
                if (lastFetchVersion === fetchVersion) {
                    ...
                    delete response.data.reload_trigger;
    ...
    const reloadTrigger = data => {
        ...
        websocketServer.dataUpdateEvent.subscribe({
            ...
            onTriggerred: () => {
                if (wsLastFetch.current === null || wsLastFetch.current.toEpochSecond() + 5 <= ZonedDateTime.now().toEpochSecond()) {
...
function getComputedQueryString(filters, columns, fetch, mode) {
    ...
    for (let key in filters.input) {
        ...
        const column = columns.find(column => column.value === key || (Array.isArray(column.searchFields) && column.searchFields.includes(key)));

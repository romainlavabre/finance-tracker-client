...
export default function ({
    ...
    const updateSubject = async field => {
        ...
        if (field.type === "boolean") {
            ...
            payload[entity.replace(regex, replaceBy)][field.name] = currentValue.current.value !== null && currentValue.current.value === "" ? null : (!isNull(field.beforeUpdate) ? field.beforeUpdate(currentValue.current.value) : currentValue.current.value);
        }
            ...
            <div className={!isNull(field.title) ? 'mt-5' : ''}>
                <div className="w-full my-auto">
                    ...
                    <div className="flex justify-between">
                        <SelectSearch2
                            key={subject + "-" + id + "-" + subjectEntity[field.name]}
                            items={field.items}
                            index={field.key}
                            value={field.value}
            ...
            <div className={!isNull(field.title) ? 'mt-5' : ''}
                ...
                <div className="flex justify-between">
                    <input
                        ...
                        autoFocus={!isNull(field.focus) && field.focus}
                    />
                    {
                        !isNull(field.disabled) && field.disabled === true
                            ? (
    ...
    return (
        <div className={!isNull(field.title) ? 'mt-5' : ''} key={subject + "-" + id + "-" + subjectEntity[field.name]}>
            {
                !isNull(field.title)
                    ? <label>{typeof field.title === "function" ? field.title(subjectEntity) : field.title}</label>
                    : null

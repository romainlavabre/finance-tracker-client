...
export default function ({
    ...
    const onSearch = () => {
        ...
        setResult(items.filter(
            item =>
                (typeof value === "function" ? value(item).toLowerCase().includes(searchInput.current.value.toLowerCase()) : item[value].toLowerCase().includes(searchInput.current.value.toLowerCase())) && !isContentContains(item))
        );
    }
